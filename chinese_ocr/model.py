...
def crnnRec(im,boxes,leftAdjust=False,rightAdjust=False,alph=0.2,f=1.0):
   ...
   for index,box in enumerate(boxes):
       ...
       #text = crnnOcr(partImg.convert('L')) # 生成一行识别结果
       text = ocr_predict.recognize(partImg.convert('L')) # 生成一行识别结果
       if text.strip()!=u'':
...
def model(img,detectAngle=False,config={},leftAdjust=False,rightAdjust=False,alph=0.2):
    ...
    config['img'] = img
    t = time.time()
    text_recs = text_detect(**config)##文字检测 生成2维数组,一行文字对应一行数据.
    print("text detect complete, it took {:.3f}s".format(time.time() - t))
